...
def main():
    ...
    while True:
        ...
        #debug_image[0:iheight, 0:iwidth] =img6
        if feel>=100 and feel<500:
            debug_image[0:iheight, 0:iwidth] = img2
            '''
            for t in range(len(images1)):
                images1[t] = cv.resize(images1[t] , (iwidth, iheight))
                debug_image[0:iheight, 0:iwidth] = images1[t]
                cv.waitKey(int(1000/fps1))  # １コマを表示するミリ秒
                '''
            if b%2==0:
