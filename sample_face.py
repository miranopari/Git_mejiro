...
kwidth=350
kheight=350
M = cv.imread('./img/mask1.png')
M = cv.resize(M , (kwidth, kheight))
...
def main():
    ...
    while True:
        ...
        # 描画 ################################################################
        draw_landmarks(image, landmarks, debug_image)
        '''
...
def draw_landmarks(image, landmarks, debug_image):
    image_width, image_height = image.shape[1], image.shape[0]
    ...
    if len(landmark_point) > 0:
        ...
        cv.line(image, landmark_point[324], landmark_point[308], (0, 255, 0),
                2)
        Pa = M
        debug_image = Paste(debug_image,Pa,landmark_point)
...
#ランダムな画像を貼り付ける関数
def Paste(debug_image,Pa,landmark_point):
        x=landmark_point[158]
        y=landmark_point[160]
        angle = 0
        scale = 1
